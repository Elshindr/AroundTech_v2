...
import ModalDeleteNatureComponent from '../../Modules/modals/ModalDeleteNatureMission';
import ModalRootComponent from '../../Modules/modals/modalRootComponent';
import ModalChildComponent from '../../Modules/modals/modalChildAddNatureComponent';
import ModalChildEditComponent from '../../Modules/modals/modalChildEditNatureComponent';

import DeleteIcon from '@mui/icons-material/Delete';
import AddCircleIcon from '@mui/icons-material/AddCircle';
import EditIcon from '@mui/icons-material/Edit';

import NatureMissionService from '../../../Services/NatureService';
...
export default function NatureComponent(props) {
    ...
    async function handleEditNature(event) {
        ...
        if (!name || isNaN(tjm) || isNaN(percentage) || (tjm < 1 && tjm !== null) || (percentage > 10 && percentage !== null)) {
            ...
            if (response === true) {
                const dataNature = await NatureMissionService.loadNaturesMis();
    ...
    const onClickModalEdit = (nature_mission) => {
        ...
        setModalChild((
            <ModalChildEditComponent
                ...
                oldInputIsCharge={nature_mission.charge}
                oldInputIsBonus={nature_mission.bonus}
                oldInputTjm={nature_mission.tjm}
                oldInputPercentage={nature_mission.percentage}
                oldInputEndDate={nature_mission.endDate}
            />
        ));
        setModalShow(true);
                            ...
                            {dataPageNatMission.map((nature_mission) => {
                                return (
                                    <tr key={nature_mission.id}>
                                        ...
                                        <td className="align-middle">
                                            {nature_mission.startDate !== null ? Utils.formatDateTimestampToStr(nature_mission.startDate) : "-"}
                                        </td>
                                        <td className="align-middle">
                                            {nature_mission.endDate !== null ? Utils.formatDateTimestampToStr(nature_mission.endDate) : "-"}
                                        </td>
                                        <td>
