...
console.log(`  nature in pdf`, nature)
  ...
  doc.text(`Date de début : ${Utils.formatDateTimestampToStr(missionData.startDate)}`, 14, 32);
  doc.text(`Date de fin : ${Utils.formatDateTimestampToStr(missionData.endDate)}`, 14, 38);
  doc.text(`Nature de la mission : ${missionData.natureCur.name}`, 14, 44);
  doc.text(`Ville de départ : ${missionData.departCity.name}`, 14, 50);
  doc.text(`Ville d'arrivée : ${missionData.arrivalCity.name}`, 14, 56);
  doc.text(`Prime : ${Utils.getPrime(missionData, nature)}€`, 14, 62);

  // Tableau des frais
  const expenseTableData = expenses.map(expense => [
    Utils.formatDateTimestampToStr(expense.createdAt),
    expense.motif.name,
  ...
  doc.save(`note-de-frais-${Utils.formatDateTimestampToStr(missionData.startDate)}-${Utils.formatDateTimestampToStr(missionData.endDate)}.pdf`);
}